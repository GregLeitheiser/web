...
develop:
	docker.exe run --mount src=$(PWD)\\public-html,target=/usr/local/apache2/htdocs,type=bind -dit --env-file=./config.env -p=$(EXT_PORT):$(INT_PORT) --name="$(APP_NAME)" servantscode/$(APP_NAME)
